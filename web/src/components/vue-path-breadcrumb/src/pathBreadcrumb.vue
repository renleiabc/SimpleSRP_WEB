...
<template>
  <p :class="cname"
    ...
    <span v-for="(item,index) of pathData"
          :key="index">
      <a href="javascript:void(0)" :title="allFileLabel"
         v-if="index===0 && index!==pathData.length-1"
         @click="gotoPath(item)">{{allFileLabel}}</a>
      <i v-else-if="index === 0 && index===pathData.length-1" :title="allFileLabel">{{allFileLabel}}</i>
      <a href="javascript:void(0)"
         :title="item.name"
         v-else-if="index>0 && (index!==pathData.length-1)"
         @click="gotoPath(item)">{{item.name}}</a>
      <i v-else-if="index>0 && index===pathData.length-1" :title="item.name">{{item.name}}</i>
