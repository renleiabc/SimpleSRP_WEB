 ...
 * @LastEditors: renlei
 * @LastEditTime: 2020-06-23 14:07:05
 * @Description:登录界面
 -->
<template>
  <div class="login-page" @keyup.enter="login">
    <iframe src="../static/loginBg/index.html" id="webglpage"></iframe>
    <transition name="form-fade" mode="in-out">
      <div class="manage-tip animated" :class="loading ? 'fadeOut' : 'fadeIn'">
        <img
          class="logo"
          :src="mixinGetImage('views/global/', 'login_logo.png')"
        />
        <h1>{{ $t("views.login.title") }}</h1>
        <div class="login-input">
          <p class="input input-uname">
            <input
              type="text"
              v-model="username"
              maxlength="16"
          ...
          <p class="input input-pwd">
            <el-tooltip
              :content="$t('views.login.capsLock')"
              v-model="capsLock"
              :manual="true"
              placement="right"
              effect="light"
          ...
          <a @click="login" class="submit-btn">{{
    ...
    <div class="loading animated fadeIn" v-if="loading">
      <div class="loader">
    ...
    <p class="footer animated" :class="loading ? 'fadeOut' : 'fadeIn'">
...
<script>
import { getQueryString } from "@/util/filters";
    ...
    setPermissionList(res) {
      ...
      if (res.code === 200) {
        ...
        const permissionTemplate = {
          ...
          storageSpaces: "8-8",
          storagePersonalLog: "8-9",
          storageProjectLog: "8-10",
          imageManagement: "9",
          publicImage: "9-1",
          objectImage: "9-2",
          personalImage: "9-3",
        ...
        breadCrumbs.push({
          name: navInformation[0].name,
          linkName: navInformation[0].linkName
        });
        if (navInformation[0].children.length > 0) {
          breadCrumbs.push({
            name: navInformation[0].children[0].name,
            linkName: navInformation[0].children[0].linkName
          });
        }
        // 填充数据
        ...
        sessionStorage.setItem(
          "navActiveChildIndex",
          navInformation[0].children.length > 0 ? "1-1" : ""
        );
