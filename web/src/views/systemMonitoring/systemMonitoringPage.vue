/* * @Author: 少侠 * @Date: 2018-07-31 10:51:17 * @LastEditors: 少侠 *
@LastEditTime: 2018-08-10 11:33:00 * @Description: 系统监控页面 */

<template>
  <div class="systemMonitoringPage">
    <div class="v-line">
      <h6>{{ $t("views.systemMonitoring.systemSource.title") }}</h6>
      <div class="record-detail" @click="goRecordList">
      ...
      <el-row>
        <el-col class="text-center" :span="8">
          <div class="chart-title">
            CPU <span class="real-value">{{ cpuUsedNum }} Core</span> /
          ...
          <div class="chart-item">
            <sedu-echarts domId="cpu" :cname="cpuClass" :option="cpuOption" />
            <div class="label-box" v-show="hasData">
              ...
              <img
                :src="mixinGetImage('views/global/', 'bottom_line.png')"
                alt=""
                class="bottom-line"
              />
              <div class="bottom-label">
                <div @click="goPersonList">
                  {{ $t("views.systemMonitoring.personal.label") }}
                  <span class="link-text">{{ personalCpu.toFixed(1) }}%</span>
                </div>
                <div @click="goProjectList">
                  {{ $t("views.systemMonitoring.project.label") }}
                  <span class="link-text">{{ projectCpu.toFixed(1) }}%</span>
                </div>
                <!-- <div>{{$t("views.systemMonitoring.systemSource.unUsed")}}
                  <span>{{(cpuAssigned-personalCpu-projectCpu).toFixed(1)}}%</span>
                </div> -->
              </div>
            </div>
          </div>
        ...
        <el-col class="text-center" :span="8">
          <div class="chart-title">
            {{ $t("views.systemMonitoring.systemSource.memory") }}
              ...
              <div class="bottom-label">
                <div @click="goPersonList">
                  ...
                  <span class="link-text"
                    >{{ personalMemory.toFixed(1) }}%</span
                  >
                </div>
                <div @click="goProjectList">
                  {{ $t("views.systemMonitoring.project.label") }}
                  <span class="link-text">{{ projectMemory.toFixed(1) }}%</span>
                </div>
                <!-- <div>{{$t("views.systemMonitoring.systemSource.unUsed")}}
                  <span>{{(memoryAssigned-personalMemory-projectMemory).toFixed(1)}}%</span>
                </div> -->
              </div>
            </div>
          </div>
        ...
        <el-col class="text-center" :span="8">
          <div class="chart-title">
            GPU <span class="real-value">{{ gpuUsedNum }} 个</span> /
          ...
          <div class="chart-item">
            ...
            <div class="label-box" v-show="hasData">
              ...
              <div class="bottom-label">
                <div @click="goPersonList">
                  {{ $t("views.systemMonitoring.personal.label") }}
                  <span class="link-text">{{ personalGpu.toFixed(1) }}%</span>
                </div>
                <div @click="goProjectList">
                  {{ $t("views.systemMonitoring.project.label") }}
                  <span class="link-text">{{ projectGpu.toFixed(1) }}%</span>
                </div>
                <!-- <div>{{$t("views.systemMonitoring.systemSource.unUsed")}}
                  <span>{{(gpuAssigned-personalGpu-projectGpu).toFixed(1)}}%</span>
                </div> -->
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    chartInit(data) {
      ...
      let assignedResource = this.$t(
        "views.systemMonitoring.systemSource.assignedResource"
      );
    ...
    pieChartInit(data) {
      ...
      let personalImage = this.$t(
        "views.systemMonitoring.images.personalImage"
      );
      let projectImage = this.$t("views.systemMonitoring.images.projectImage");
      let publicStorage = this.$t(
        "views.systemMonitoring.storages.publicStorage"
      );
      let personalStorage = this.$t(
        "views.systemMonitoring.storages.personalStorage"
      );
      let projectStorage = this.$t(
        "views.systemMonitoring.storages.projectStorage"
      );
      let bianImage = "编排镜像";
      this.option5 = {
        title: {
        ...
        series: [
          {
            ...
            data: [
              ...
              { value: data.imagesCount.bianNum, name: bianImage }
            ]
          }
        ],
        color: ["#f25d59", "#83dfc8", "#ba99bc", "#f4b54f"]
      };
      this.option6 = {
        title: {
  ...
  created() {
    ...
    this.$get("/system/hypervisor").then(res => {
      ...
      data["imagesCount"] = {
        ...
        bianNum: res.data.bian_image_count_total || 0
      };
    ...
    this.$get("/system/getsystemresources").then(res => {
      ...
      /* 已使用 */
      this.cpuAssigned = this.cpuAllNum
        ? (this.cpuUsedNum / this.cpuAllNum) * 100
        : 0;
      this.memoryAssigned = this.memoryAllNum
        ? (this.memoryUsedNum / this.memoryAllNum) * 100
        : 0;
      this.gpuAssigned = this.gpuAllNum
        ? (this.gpuUsedNum / this.gpuAllNum) * 100
        : 0;
      /* 未使用 */
      ...
      /* 水球图 个人空间、项目空间 */
      this.personalCpu = this.cpuAllNum
        ? (data.personal_ns_cpu_use / this.cpuAllNum) * 100
        : 0;
      this.projectCpu = this.cpuAllNum
        ? (data.project_ns_cpu_use / this.cpuAllNum) * 100
        : 0;

      this.personalMemory = this.memoryAllNum
        ? (data.personal_ns_memory_use / this.memoryAllNum) * 100
        : 0;
      this.projectMemory = this.memoryAllNum
        ? (data.project_ns_memory_use / this.memoryAllNum) * 100
        : 0;

      this.personalGpu = this.gpuAllNum
        ? (data.personal_ns_gpu_use / this.gpuAllNum) * 100
        : 0;
      this.projectGpu = this.gpuAllNum
        ? (data.project_ns_gpu_use / this.gpuAllNum) * 100
        : 0;
      /* 水球图的数据，要排序(降序)后才能展示，不然数据小的会被覆盖 （个人空间、项目空间、未使用） */
      data["cpuData"] = [];
      data["memoryData"] = [];
      data["gpuData"] = [];
      if (this.personalCpu > 0 || this.projectCpu > 0) {
        data["cpuData"].push(this.personalCpu / 100, this.projectCpu / 100);
        data["cpuData"].sort(function(a, b) {
          return b - a;
        });
      }
      if (this.personalMemory > 0 || this.projectMemory > 0) {
        data["memoryData"].push(
          this.personalMemory / 100,
          this.projectMemory / 100
        );
        data["memoryData"].sort(function(a, b) {
          return b - a;
        });
      }
      if (this.personalGpu >= 0 || this.projectGpu > 0) {
        data["gpuData"].push(this.personalGpu / 100, this.projectGpu / 100);
