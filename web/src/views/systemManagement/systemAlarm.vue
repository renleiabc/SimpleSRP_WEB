 ...
 * @LastEditors: renlei
 * @LastEditTime: 2020-06-16 17:37:08
 * @Description:系统告警
-->
  ...
  <div class="views-system-management-system-alarm">
    <div class="tab-select">
      <div>
        <span class="tab-select-title">{{
          $t("views.systemManagement.nodeName")
        }}</span>
        <el-select
          v-model="tableData.sel.ip"
          :placeholder="$t('views.systemManagement.chose')"
        >
          <el-option
            v-for="item in node_name"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <div>
        <span class="tab-select-title">{{
          $t("views.systemManagement.warnDate")
        }}</span>
        <el-date-picker
      ...
      <div>
        <span class="tab-select-title">{{
          $t("views.systemManagement.warnStates")
        }}</span>
        <el-select
          v-model="tableData.sel.status"
          :placeholder="$t('views.systemManagement.chose')"
        >
          <el-option
            v-for="item in warn_status"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <div>
        <span class="tab-select-title">{{
          $t("views.systemManagement.warnType")
        }}</span>
        <!-- 告警类型 -->
        <el-select
          v-model="tableData.sel.type"
          :placeholder="$t('views.systemManagement.chose')"
        >
          <el-option
            v-for="item in warn_type"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <div>
        <el-button class="btn light small" @click="clearSearch('tableData')">{{
          $t("views.systemManagement.reset")
        }}</el-button>
        <el-button class="btn light small" @click="doSearch('tableData')">{{
          $t("views.systemManagement.select")
        }}</el-button>
        <!-- 提示规则 -->
        <el-tooltip placement="bottom" effect="light">
          <el-button class="btn light small"> 规则 </el-button>
          <div slot="content" class="system-tip">
            系统告警根据阀值来算,如CPU低位阀值50
            <br />
            中位阀值50+(100-50)/3 = 66.7
            <br />
            高位阀值50+ 2((100-50)/3) = 83.3
            <br />
            50-66.7为低风险,66.7-83.3为中风险,83.3以上为高风险
          </div>
        </el-tooltip>
      </div>
    </div>
    ...
    >
      ...
      <el-table-column
        ...
        show-overflow-tooltip
        align="center"
        :label="$t('views.systemManagement.nodeName')"
      ></el-table-column>
      <el-table-column
        prop="type"
        align="center"
        width="100"
        show-overflow-tooltip
        :label="$t('views.systemManagement.warnType')"
      ></el-table-column>
      <el-table-column
        prop="content"
        show-overflow-tooltip
        :label="$t('views.systemManagement.warnContent')"
      ></el-table-column>
      <el-table-column
        :label="$t('views.systemManagement.warnStates')"
        width="100"
        align="center"
      >
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.status == 1" class="green">{{
            $t("views.systemManagement.warnLevel1")
          }}</span>
          <span v-if="scope.row.status == 2" class="yellow">{{
            $t("views.systemManagement.warnLevel2")
          }}</span>
          <span v-if="scope.row.status == 3" class="red">{{
            $t("views.systemManagement.warnLevel3")
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      if (this[this.linTabData].sel !== undefined) {
        if (
          ...
          this.tableData.sel.start_time =
            this.tableData.sel.full_date[0] + " 00:00:00";
          this.tableData.sel.end_time =
            this.tableData.sel.full_date[1] + " 23:59:59";
        } else {
