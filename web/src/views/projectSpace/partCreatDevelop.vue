 ...
 * @LastEditors: renlei
 * @LastEditTime: 2020-06-06 17:11:25
  ...
  <div class="views-personal">
    ...
    >
      ...
      <div class="views-header-inf">
        <span>{{ $t("views.development.developingInformation") }} </span>
      </div>
      <div class="views-develop-inf">
        <div class="views-develop-text">
          ...
          <div class="views-develop-image">
            <el-form-item
              :label="$t('views.development.environmentalName')"
              prop="name"
            >
          ...
          <div class="views-develop-image">
            ...
            >
              <div class="views-develop-choice">
                <el-input v-model="ruleForm.image" :readonly="true"></el-input>
                <el-button
                  type="primary"
                  class="view-btn-medium"
                  @click="classification('ruleForm')"
                  >{{ $t("views.button.choice") }}</el-button
                >
              </div>
            </el-form-item>
          </div>
          <div class="views-develop-image">
            <el-form-item
              :label="$t('views.development.selectMirror')"
              prop="mirror"
            >
              <div class="views-develop-choice">
      ...
      <div class="views-develop-inf">
        <div class="views-develop-text">
          <div class="views-develop-input">
            <el-form-item
              :label="$t('views.development.pleasePassword')"
              prop="pass"
            >
              <el-input
        ...
        <div class="views-develop-text">
          <div class="views-develop-input">
            <el-form-item
              :label="$t('views.development.confirmPassword')"
              prop="checkPass"
            >
              <el-input
      ...
      <div class="views-develop-mount">
        <el-form-item
          v-for="(item, index) in ruleForm.address"
          :key="index"
          :rules="{
            required: true,
            message: '挂载信息不能为空',
      ...
      <div class="views-develop-mount">
        ...
        <span class="views-mount-span"
          >*{{ $t("views.development.mountableAddresses") }}</span
        >
      </div>
    ...
    <!-- 选择镜像 -->
    <el-dialog
      :title="$t('views.development.selectMirror')"
    ...
    <!-- 镜像分类弹出框 -->
    <el-dialog
      :title="$t('views.development.classification')"
    ...
    >
      <div class="content">
        <views-code-file
          @selectStroage="showStroage($event)"
          :projectId="projectId"
        ></views-code-file>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button class="cancel" @click="dialogAddress = false">{{
          $t("views.button.cancel")
        }}</el-button>
        <el-button class="primary" type="primary" @click="dialogSure">{{
          $t("views.button.sure")
        }}</el-button>
      </span>
    </el-dialog>
...
export default {
  data() {
    ...
    var firstPass = (rule, value, callback) => {
      if (value === "") {
        ...
        callback(
          new Error("请输入6-16位字母、数字、特殊字符(!@#$%^&*)，至少两种组合")
        );
      } else {
        if (this.ruleForm.checkPass !== "") {
          this.$refs.ruleForm.validateField("checkPass");
      ...
      createDevrules: {
        ...
        pass: [
          { required: true, validator: firstPass, trigger: "blur" },
          {
            min: 6,
            max: 16,
            message: this.$t("views.development.password"),
            trigger: "blur"
          }
        ],
        checkPass: [
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        ...
        if (valid) {
          ...
          let mountInfo = this.handleMountInfo(address, storageUser);
          let create = {
    ...
    // 挂载存储信息的数据重组封装
    handleMountInfo(arr, storageUser) {
      if (arr.length === 0) {
        this.mount_info = {};
      } else {
        var obj = {};
