 ...
 * @LastEditors  : renlei
 * @LastEditTime : 2020-02-14 10:34:24
...
export default {
  data() {
    ...
    var checkAge = (rule, value, callback) => {
      ...
      const reg = /^[1-9]\d*$/
      const regGpu = /^([0]|[1-9][0-9]*)$/
      switch (field) {
        case 'cpu':
          if (!reg.test(value)) {
            callback(new Error('请输入正整数'))
          } else if (value > limitResource.limit_cpu) {
          ...
          break
        case 'gpu':
          if (!regGpu.test(value)) {
            callback(new Error('请输入0或正整数'))
          } else if (value > limitResource.limit_gpu) {
        ...
        case 'memory':
          if (!reg.test(value)) {
            callback(new Error('请输入正整数'))
          } else if (value > limitResource.limit_memory) {
