 ...
 * @LastEditors: renlei
 * @LastEditTime: 2020-06-23 16:32:37
  ...
  <div class="views-personal">
    ...
    >
      ...
      <div class="views-develop-inf views-model-image">
        <div class="views-task-input" v-if="false">
        ...
        <div class="views-develop-resources">
          ...
          <!-- 版本信息 -->
          <el-form-item
            :label="$t('views.trainingTask.versionInf')"
            prop="version"
          >
            <el-select
          ...
          <div class="views-task-show" v-show="tensorflow_show">
            <span class="views-task-radio ">{{
              $t("views.trainingTask.whetherVisualization")
            }}</span>
        ...
        <!-- caffe运行参数-->
        <div class="views-develop-image views-caffe-required" v-if="caffe_show">
          <el-form-item
            :label="$t('views.trainingTask.operatParameters')"
            prop="params"
          >
            <div class="views-develop-params">
              <div
                class="views-develop-caffe"
              ...
              >
                ...
                >
                  ...
                  <el-input
                    ...
                    v-else
                  ></el-input>
                  <span class="views-task-tips" v-show="item.isShow">{{
                    item.tip
                  }}</span>
                </el-form-item>
              </div>
        ...
        <!-- tensorflow运行参数-->
        <div class="views-develop-image" v-if="tensorflow_show">
          <el-form-item
            :label="$t('views.trainingTask.operatParameters')"
            prop="params"
          >
            <div class="views-develop-params">
              ...
              >
                ...
                <el-input
                  v-model="item.value"
                  @input="tensorflowBlur($event, index)"
                ></el-input>
                <span class="views-task-tips" v-show="item.isShow">{{
                  item.tip
                }}</span>
        ...
        <div class="views-develop-image views-task-mb" style="display:none">
          ...
          >
            <el-form-item
              v-for="(item, index) in ruleForm.operating"
              :key="index"
              :rules="{
                required: true,
                message: '输入不能为空',
            ...
            >
              <div class="views-develop-choice">
                ...
                <span
                  class="views-btn-reduce "
                  @click="reduceAddress(val, index)"
                  >—</span
                >
              </div>
            </el-form-item>
            <div class="views-develop-choice">
              <el-button
                ...
                @click="operatParams"
                >{{ $t("views.trainingTask.increaseParameters") }}</el-button
              >
            </div>
          </el-form-item>
        </div>
      ...
      <div class="views-task-type ">
        <div class="views-task-right views-task-border ">
          <!-- caffe节点配置 -->
          <div
            class="views-task-sitting views-task-sitting-mg "
            v-show="caffe_show"
          >
            ...
            >
              <el-option
                v-for="item in projectconfig.cpu_config"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              >
            ...
            >
              <el-option
                v-for="item in projectconfig.gpu_config"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              >
            ...
            >&nbsp;个
            <span class="views-task-cpu ">{{
              $t("views.projectContent.memory")
            }}</span>
            <el-select
              v-model="caffe.memory"
              :placeholder="$t('views.development.pleaseChoose')"
            >
              <el-option
                v-for="item in memory_conifg"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              >
          ...
          <div class="views-task-tensorflow" v-show="tensorflow_show">
            ...
            <div class="views-task-node ">
              ...
              <div class="views-task-sitting views-task-select ">
                ...
                <span class="views-task-cpu ">CPU</span>
                <el-select
                  v-model="tensorflow.ps_node.cpu"
                  :placeholder="$t('views.development.pleaseChoose')"
                >
                  <el-option
                    v-for="item in projectconfig.cpu_config"
                ...
                <span class="views-task-cpu ">GPU</span>
                <el-select
                  v-model="tensorflow.ps_node.gpu"
                  :placeholder="$t('views.development.pleaseChoose')"
                >
                  <el-option
                    v-for="item in projectconfig.gpu_config"
                ...
                <el-select
                  v-model="tensorflow.ps_node.memory"
                  :placeholder="$t('views.development.pleaseChoose')"
                >
                  <el-option
                    v-for="item in projectconfig.memory_config"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  >
              ...
              <div class="views-task-node views-task-bottom ">
                ...
                <div class="views-task-sitting views-task-select">
                  ...
                  <el-input
                    ...
                    clearable
                  ></el-input>
                  <span class="views-task-tips" v-show="workerShow">{{
                    workerTip
                  }}</span>
                  <!-- worker节点 CPU-->
                  <span class="views-task-cpu">CPU</span>
                  <el-select
                    v-model="tensorflow.work_node.cpu"
                    :placeholder="$t('views.development.pleaseChoose')"
                  >
                    <el-option
                      v-for="item in projectconfig.cpu_config"
                  ...
                  <span class="views-task-cpu ">GPU</span>
                  <el-select
                    v-model="tensorflow.work_node.gpu"
                    :placeholder="$t('views.development.pleaseChoose')"
                  >
                    <el-option
                      v-for="item in projectconfig.gpu_config"
                  ...
                  <el-select
                    v-model="tensorflow.work_node.memory"
                    :placeholder="$t('views.development.pleaseChoose')"
                  >
                    <el-option
                      v-for="item in memory_conifg"
      ...
      <div class="views-develop-btn ">
        <el-button @click="resetForm('ruleForm')">
          {{ $t("views.button.cancel") }}</el-button
        >
        <el-button
          type="primary "
          class="view-btn-big"
      ...
      >
        <div class="content">
          <views-code-directory
            ...
            :type="item.type"
            :projectId="projectId"
            @storageUserOn="handleStorageUserOn($event)"
          ></views-code-directory>
        </div>
        <span slot="footer" class="dialog-footer">
          <el-button class="cancel" @click="dialogColse">{{
            $t("views.button.cancel")
          }}</el-button>
          <el-button
            class="primary"
            type="primary"
            @click="dialogSure(index)"
...
export default {
  data() {
    ...
    return {
      ...
      workerShow: true,
      isSecond: false,
      caffe: { cpu: 1, gpu: 0, memory: 8 },
      tensorflow: {
        ps_node: { num: 1, cpu: 1, gpu: 0, memory: 4 },
        work_node: { num: 1, cpu: 1, gpu: 0, memory: 8 }
      },
      memory_conifg: [
        { label: 8, value: 8 },
        { label: 16, value: 16 },
      ...
      ruleForm: {
        ...
        choice: [
          {
            title: this.$t("views.trainingTask.codeDirectory"),
            dialogCode: false,
            isFirst: false,
            isSecond: false,
            type: 1,
          ...
          {
            title: this.$t("views.trainingTask.trainingData"),
            dialogCode: false,
            isFirst: false,
            isSecond: false,
            type: 1,
          ...
          {
            title: this.$t("views.trainingTask.startupFile"),
            dialogCode: false,
            isFirst: false,
            isSecond: false,
            type: 1,
          ...
          {
            title: this.$t("views.trainingTask.trainingLog"),
            dialogCode: false,
            isFirst: false,
            isSecond: false,
            type: 1,
      ...
      createDevrules: {
        name: [
          { required: true, validator: name, trigger: "blur" },
          {
            min: 2,
            max: 30,
            message: this.$t("views.development.youCan"),
            trigger: "blur"
          }
        ],
        engine: [
        ...
        desc: [{ required: false, validator: desc, trigger: "blur" }],
  ...
  methods: {
    ...
    useData(lastData) {
      ...
      switch (data.engine) {
        ...
        case "caffe":
          ...
          this.ruleForm.caffeChoice[0].input =
            data.mount_info.train_data_dir.s3_path;
          this.ruleForm.caffeChoice[1].input =
            data.mount_info.test_data_dir.s3_path;
          this.ruleForm.caffeChoice[2].input =
            data.mount_info.proto_file.s3_path;
          this.ruleForm.caffeChoice[3].input =
            data.mount_info.snapshot_prefix.s3_path;
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.ruleForm.engine === "caffe") {
            this.postCaffe.snapshot_prefix = {
              mount_flag: "rw",
    ...
    getPracticeparam() {
      ...
      this.$post("/practiceproject/practiceparam", obj).then(res => {
        if (res.code === 200) {
          this.s3_path = this.ruleForm.caffeChoice[3].input =
            res.data.output_dir;
          this.s3_path = this.ruleForm.choice[3].input = res.data.output_dir;
