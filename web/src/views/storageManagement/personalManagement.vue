 ...
 * @LastEditors: renlei
 * @LastEditTime: 2020-06-02 18:53:41
 * @Description:项目存储管理
-->
  ...
  <div class="announcement">
    ...
    <div class="controls-else">
      <el-button class="view-btn-medium" @click="changeStorage">{{
        $t("views.storageManagement.storage.btnCreatePer")
      }}</el-button>
      <el-button
        class="view-btn-medium"
        :disabled="tableData.msg.length == 0"
        @click="saveAuthenticate"
        >{{
          $t("views.storageManagement.storage.btnSaveAuthenticate")
        }}</el-button
      >
      <el-button
        class="view-btn-medium view-btn-right"
        :disabled="tableData.msg.length == 0"
        @click="personalStorage"
        >{{
          $t("views.storageManagement.storage.btnPersonalManege")
        }}</el-button
      >
    </div>
    <el-table
    ...
    >
      ...
      >
        <template slot-scope="item">
          <span v-if="item.row.status == 0">{{ item.row.php_name }}</span>
          <router-link
            v-else
            class="link"
            :to="{
      ...
      <el-table-column :label="$t('views.table.operate')" width="140">
    ...
    <el-dialog
      :title="$t('views.storageManagement.storage.btnCreatePer')"
...
export default {
  ...
  methods: {
    ...
    // 个人日志存储空间
    personalStorage() {
      this.$router.push({ name: "storagePersonalLog" });
    },
    clearCreatInfo() {
      this.isDisabel = false;
