/* * @Author: Liang Liang * @Date: 2018-08-11 14:29:09 * @LastEditors:任雷雷 *
@LastEditTime: 2018-08-11 16:12:21 * @Description:项目存储管理 */ /* *
@LastEditors: aFei * @LastEditTime: 2018-08-22 11:31:55 */
<template>
  <div class="announcement">
    <div class="tab-select">
      <div>
        <span class="tab-select-title">{{
          $t("views.storageManagement.storage.storageName")
        }}</span>
        <el-input
          v-model="tableData.sel.php_name"
          :placeholder="$t('views.storageManagement.storage.storageName')"
          class="views-input-text"
        ></el-input>
      </div>
      <div>
      ...
      <div>
        <span class="tab-select-title">{{
          $t("views.storageManagement.storage.belongSpace")
        }}</span>
        <el-select
          v-model="tableData.sel.project_id"
          clearable
          filterable
      ...
      <div>
        <el-button class="btn light small" @click="clearSearch('tableData')">{{
          $t("views.button.reset")
        }}</el-button>
        <el-button class="btn light small" @click="doSearch('tableData')">{{
          $t("views.button.search")
        }}</el-button>
      </div>
    </div>
    <div class="controls-else">
      <el-button class="view-btn-medium" @click="changeStorage">{{
        $t("views.storageManagement.storage.btnCreatePro")
      }}</el-button>
    </div>
    <el-table
      class="tab-content"
    ...
    >
      ...
      <el-table-column
        :label="$t('views.storageManagement.storage.storageName')"
        prop="php_name"
      ...
      >
        <template slot-scope="item">
          <router-link
            class="link"
            v-if="item.row.is_archive == 0 && item.row.status == 1"
            :to="{
              name: 'proStorageDetail',
              query: { id: item.row.id, is_init: tableData.sel.is_init }
            }"
            >{{ item.row.php_name }}</router-link
          >
      ...
      <el-table-column
        ...
        sortable="custom"
        align="center"
        :label="$t('views.table.createTime')"
      ></el-table-column>
      <el-table-column
        prop="project_name"
        show-overflow-tooltip
        :label="$t('views.storageManagement.storage.belongSpace')"
      ></el-table-column>
      <el-table-column label="项目状态" align="center" prop="is_archive">
        <template slot-scope="item">
      ...
      <el-table-column
        prop="describe"
        show-overflow-tooltip
        :label="$t('views.table.describe')"
      >
        <template slot-scope="item">
          <span v-if="item.row.status == 0" style="color:#ff7989;"
            >无效的存储空间</span
          >
          <span v-else>{{ item.row.describe }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('views.table.operate')" width="300">
        <div class="options text-center" slot-scope="item">
          <button
            class="btn"
            v-if="item.row.status == 1"
            @click="projectLog(item.row.id)"
          >
            项目日志
          </button>
          <button
            class="btn"
            v-if="item.row.status == 1"
            @click="fileManage(item.row.id)"
          >
            {{ $t("views.storageManagement.storage.btnFileManege") }}
          </button>
          <button
            class="btn"
            v-if="item.row.status == 1"
          ...
          <button class="del" @click="deleteStorage(item.row.id)">
    ...
    <el-dialog
      :title="$t('views.storageManagement.storage.btnCreatePro')"
    ...
    >
      <div>
        <div class="create-storage">
          <span>
            <i style="color:red;">*</i
            >{{ $t("views.storageManagement.storage.spaceName") }}</span
          >
          <el-input
            v-model="createStorage.storage_name"
            maxlength="30"
          ></el-input>
        </div>
        <div class="create-storage">
          <span>
            <i style="color:red;">*</i
            >{{ $t("views.storageManagement.storage.selectSpace") }}</span
          >
          <el-select
            v-model="createStorage.project_id"
            :placeholder="$t('views.imageManagement.chose')"
          >
            <el-option
              v-for="item in arcProList"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="createStorageDialog = false" size="medium">{{
  ...
  methods: {
    ...
    saveStorage() {
      ...
      if (this.createStorage.project_id === "") {
        this.$message({
          message: this.$t("views.storageManagement.valid.notNullPro"),
          type: "error"
        });
        return false;
      }
      if (
        !lowerValid(this.createStorage.storage_name) ||
        this.createStorage.storage_name.length < 3
      ) {
        this.$message({
      ...
      var regBlank = /^[\s]+$/;
      if (regBlank.test(this.createStorage.storage_description)) {
        this.$message({ message: "只输入空格不可提交", type: "error" });
        return false;
      }
      this.isDisabel = true;
      let parms = {
        is_init: this.tableData.sel.is_init,
        php_name: this.createStorage.storage_name,
        project_id: this.createStorage.project_id,
        describe: this.createStorage.storage_description
      };
    ...
    saveAuthenticate(row) {
      ...
      link.href =
        window.Simple.php +
        "/v1/storage/downloadkey?access-token=" +
    ...
    initMsg() {
      ...
      if (this[this.linTabData].sel !== undefined) {
        if (
          ...
          this[this.linTabData].sel.start_time = this[
            this.linTabData
          ].sel.full_date[0];
          // eslint-disable-next-line standard/computed-property-even-spacing
          this[this.linTabData].sel.end_time = this[
            this.linTabData
          ].sel.full_date[1];
        } else {
      ...
      this.$post(this[this.linTabData].url + `?sort=${lin.sort}`, {
        StorageSearch: { ...lin },
        page: lin.page
      }).then(data => {
        // 根据后台接口更改
    ...
    fileManage(id) {
      this.$router.push({
        name: "proStorageDetail",
        query: { id: id, is_init: this.tableData.sel.is_init }
      });
    },
    deleteStorage(id) {
      // url请求地址  params参数  message自定义提示信息,默认提示 "确认要删除吗？"
      this.mixinConfirm("/storage/storagedelete", {
        storage_id: [id],
        force: 0
      }).then(res => {
        if (res) {
