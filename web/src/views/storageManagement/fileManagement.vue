/* * @Author: Liang Liang * @Date: 2018-08-07 14:29:09 * @LastEditors: 少侠 *
@LastEditTime: 2018-08-10 18:00:24 * @Description: */
  ...
  <div class="app-container">
    ...
    <div class="options">
      <div class="search">
        ...
        <el-input size="small" v-model="searchName"></el-input>
        <el-button class="views-btn-query" size="small" @click="resetNameBtn">{{
          $t("views.button.reset")
        }}</el-button>
        <el-button
      ...
      <div class="btn-group" v-if="showBtn">
        <el-button class="dark mid btn" @click="dialogFTP = true"
          >FTP上传</el-button
        >
        <el-button class="dark mid btn" @click="dialogNewFile = true">{{
          $t("views.storageManagement.detail.btnUpload")
        }}</el-button>
        <el-button
          class="dark mid btn"
          size="small"
          @click="createFloderDialod"
          >{{ $t("views.storageManagement.detail.btnDir") }}</el-button
        >
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <div v-if="scope.row.type == '1'">
            <i class="iconfont icon-wenjianjia" style="color:#417dd0;" />&nbsp;
      ...
      <el-table-column
        ...
        width="120"
        :label="
          $t('views.storageManagement.fileManagement.tableFileList.thEncipher')
        "
      >
        <template slot-scope="scope">
          <div v-if="scope.row.type == '1'">
          ...
          <div v-else>
            <el-tag
              v-if="scope.row.is_encryption == 2"
              type="success"
              size="small"
              >自定义密钥加密</el-tag
            >
            <el-tag
              v-if="scope.row.is_encryption == 1"
              type="success"
              size="small"
              >系统密钥加密</el-tag
            >
            <el-tag
              v-if="scope.row.is_encryption == 0"
              type="info"
              size="small"
              >{{
                $t(
                  "views.storageManagement.fileManagement.tableFileList.tagUnencrypted"
                )
              }}</el-tag
            >
          </div>
      ...
      <el-table-column
        ...
        width="120"
        :label="
          $t('views.storageManagement.fileManagement.tableFileList.thStateFile')
        "
      >
        <template slot-scope="scope">
          <div>
            <el-tag v-if="scope.row.status == 1" type="success" size="small">{{
              $t("views.storageManagement.detail.success")
            }}</el-tag>
            <el-tag v-if="scope.row.status == 0" type="info" size="small">{{
              $t("views.storageManagement.detail.loading")
            }}</el-tag>
            <el-tag v-if="scope.row.status == 2" type="error" size="small">{{
              $t("views.storageManagement.detail.fail")
            }}</el-tag>
          </div>
        </template>
      </el-table-column>
      <el-table-column
        align="center"
        :label="
          $t('views.storageManagement.fileManagement.tableFileList.thDate')
        "
        prop="update_at"
      >
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          ...
          >
            {{
              $t("views.storageManagement.fileManagement.tableFileList.btnEdit")
            }}
          </button>
          <span v-if="scope.row.dowoldstatus == 2">{{
            $t("views.storageManagement.detail.fetching")
          }}</span>
    ...
    <el-dialog
      :title="$t('views.storageManagement.detail.btnDir')"
      class="small"
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogNewFlod = false">{{
          $t("views.button.cancel")
        }}</el-button>
        <el-button type="primary" class="primary" @click="createFloder">{{
          $t("views.button.sure")
        }}</el-button>
      </div>
    </el-dialog>
    ...
    >
      <div class="ftp-box">
        <a :href="ftp_down" class="view-btn view-btn-medium a-btn"
          >点击下载FTP工具快速传输大文件</a
        >
        <p>为防止中文乱码，需上传编码为UTF-8的文本文件。</p>
        <p>
          FTP地址：{{ getterStorageUser ? getterStorageUser.ftp_path : "" }}:{{
            getterStorageUser ? getterStorageUser.ftp_port : ""
          }}
        </p>
        <p>账号：{{ getterStorageUser ? getterStorageUser.keyname : "" }}</p>
        <p>
          密码：{{ getterStorageUser ? getterStorageUser.keypassword : "" }}
        </p>
      </div>
    </el-dialog>
    <el-dialog
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          ...
          :disabled="uploading"
          @click="btnNewFile"
          >{{ $t("views.button.sure") }}</el-button
        >
        <p class="text-desc">
          {{ $t("views.storageManagement.detail.fileSize") }}
        </p>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  created() {
    ...
    this.errorString = this.$t(
      "views.storageManagement.fileManagement.tableFileList.thRecover"
    );
  ...
  methods: {
    ...
    createFloder() {
      let reg = /^[a-z0-9][\u4e00-\u9fa5a-zA-Z0-9-_.]+$/g;
      if (!reg.test(this.newFloderName) || this.newFloderName.length < 3) {
        this.$message({
          message: this.$t("views.storageManagement.valid.folderName"),
          type: "error"
        });
      } else {
        this.dialogNewFlod = false;
        this.$post("/storage/foldercreat", {
          foldername: this.newFloderName,
    ...
    getPingtai(row) {
      ...
      this.$post("/storage/objectdowdb", { fileid: row.id }).then(res => {
        if (res.code === 200) {
          ...
          this.$post(
            window.Simple.pingTai + "/oss/user/object/pull",
            parms
          ).then(res => {
            if (res.state === 200) {
              this.$message({
                message: this.$t("views.storageManagement.valid.fetchFile"),
                type: "warning"
              });
              this.searchNameBtn(1);
            } else {
    ...
    deleteFile(id) {
      this.$confirm(
        this.$t("views.dialogText.deleteConfirm"),
        this.$t("views.dialogText.tip")
      )
        .then(() => {
          this.$post("/storage/pseudofolderdelete", {
            folderid: id,
            force: 1,
            storage_id: this.storageId
          }).then(res => {
    ...
    sendPost(row) {
      ...
      if (row.size > 100 * 1024 * 1024) {
        this.$message({
          type: "warning",
          message: "下载文件大于100M，请使用FTP下载"
        });
      } else {
        let xhr;
        const parms = {
    ...
    updateTable(obj) {
      ...
      if (obj.status !== "1") {
        this.$message.warning(
          this.$t("views.storageManagement.valid.foldError")
        );
      } else {
        this.pid = obj.id;
        this.searchName = "";
    ...
    getFileListCb(obj) {
      ...
      this.$post(this.apiFileList, {
        StorageObjectSearch: prams,
    ...
    getFileList(obj, tongbu = 0) {
      ...
      this.$parent.getStorageDetail(this.storageId);
    },
    creatTimer() {
      // koufei 20181112
      this.clearTimer();
    ...
    setFileIds() {
      ...
      for (var object in this.tableData) {
        if (parseInt(this.tableData[object].status) === 0) {
          // 上传创建
          this.fileIds.push(this.tableData[object].id);
        }
        if (parseInt(this.tableData[object].dowoldstatus) === 2) {
          // 获取下载
    ...
    isGetObjectStatus() {
      ...
      if (this.fileIds.length >= 1 || this.fildowonIds.length >= 1) {
        this.$post(this.fileIdsPath, {
          obgect_id: this.fileIds,
          fildowon_ids: this.fildowonIds
        }).then(res => {
          if (res.data.returnarr.length >= 1) {
            for (let returnObj in res.data.returnarr) {
              for (let tabObj in this.tableData) {
                if (
                  parseInt(this.tableData[tabObj].id) ===
                  parseInt(res.data.returnarr[returnObj].id)
                ) {
                  var offIndex = this.fileIds.indexOf(
                    res.data.returnarr[returnObj].id
                  );
          ...
          if (res.data.returndow.length >= 1) {
            for (let returnObj in res.data.returndow) {
              for (let tabObj in this.tableData) {
                if (
                  parseInt(this.tableData[tabObj].id) ===
                  parseInt(res.data.returndow[returnObj].id)
                ) {
                  let offIndex = this.fildowonIds.indexOf(
                    res.data.returndow[returnObj].id
                  );
                  this.getFileListCb();
